...
dotnet publish ".\src\ToolTikTok.Worker\ToolTikTok.Worker.csproj" -c Release -r win-x64 --self-contained true -p:PublishSingleFile=false -p:PublishReadyToRun=false -p:PublishTrimmed=false -o "%OUT%"
if errorlevel 1 goto :fail

echo.
echo [2/3] Publish Manager self-contained win-x64...
dotnet publish ".\src\ToolTikTok.Manager\ToolTikTok.Manager.csproj" -c Release -r win-x64 --self-contained true -p:PublishSingleFile=false -p:PublishReadyToRun=false -p:PublishTrimmed=false -o "%OUT%"
if errorlevel 1 goto :fail
