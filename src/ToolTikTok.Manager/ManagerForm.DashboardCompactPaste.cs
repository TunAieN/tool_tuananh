...
public sealed partial class ManagerForm
{
    ...
    static void ConfigureMergedDashboardColumnOrder(DataGridView grid)
    {
        ...
        if (grid.Columns.Contains("Profile")) grid.Columns["Profile"].Width = 82;
        if (grid.Columns.Contains("Account")) grid.Columns["Account"].Width = 140;
        if (grid.Columns.Contains("RunState")) grid.Columns["RunState"].Width = 100;
        if (grid.Columns.Contains("SessionRuntime")) grid.Columns["SessionRuntime"].Width = 105;
        if (grid.Columns.Contains("TotalRuntime")) grid.Columns["TotalRuntime"].Width = 115;
        if (grid.Columns.Contains("RoundsPerHour")) grid.Columns["RoundsPerHour"].Width = 90;
        if (grid.Columns.Contains("Progress"))
        {
            grid.Columns["Progress"].AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill;
            grid.Columns["Progress"].MinimumWidth = 180;
        }
    }
