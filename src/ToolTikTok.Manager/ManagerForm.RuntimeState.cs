...
public sealed partial class ManagerForm
{
    ...
    void OnWorkerProcessExited(ProfileContext ctx, Process process)
    {
        void ApplyExit()
        {
            ...
            var runtimeMs = ctx.WorkerStartedUtc == DateTime.MinValue
                ? -1
                : Math.Max(0, (long)(DateTime.UtcNow - ctx.WorkerStartedUtc).TotalMilliseconds);
            var expectedStop = ctx.WorkerExpectedStop;
            var stopReason = string.IsNullOrWhiteSpace(ctx.WorkerStopReason)
                ? (exitCode == "0" ? "normal_exit" : "unexpected_exit")
                : ctx.WorkerStopReason;
            _log.Info($"[WORKER_EXIT] profile={ctx.Profile.Name} pid={SafeProcessId(process)} exitCode={exitCode} runtimeMs={runtimeMs} expectedStop={expectedStop} stopReason={stopReason}");
