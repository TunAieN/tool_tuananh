...
public sealed partial class MainForm
{
    ...
    public Task<string> HandleManagedCommandAsync(string rawCommand)
    {
        ...
        if (command == "handshake")
            return Task.FromResult(WorkerIpcProtocol.BuildHandshake(AppVersionInfo.Current, _startupOptions.ProfileName));
        ...
        return InvokeManagedOnUiAsync(async () =>
        {
            switch (command)
            {
                ...
                case "shutdown":
                    StopManagedMessageReply();
                    WorkerBootLog.Write(_baseDir, "[WORKER_STOP] reason=manager_shutdown exitCode=0");
