...
public sealed partial class ManagerForm
{
    ...
    Control BuildDashboardProfileCard(DataGridView grid)
    {
        ...
        {
            Text = "Quản lý và theo dõi trạng thái các hồ sơ trong thời gian thực.",
            AutoSize = true,
            Location = new Point(55, 33),
            ForeColor = UiTheme.TextSecondary,
            Font = new Font("Segoe UI", 8.5F)
        };
        heading.Controls.Add(_dashboardSummary);
        ...
        tools.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute, 180F));
        tools.ColumnStyles.Add(new ColumnStyle(SizeType.Absolute, 148F));
        ...
        ModernDialog.StyleTextInput(_dashboardSearchBox);
        ...
        _dashboardStatusFilter.Items.AddRange(["Tất cả trạng thái", "Đang chạy", "Tạm dừng", "Đang khôi phục", "Đã dừng", "Không xác định"]);
        _dashboardStatusFilter.SelectedIndex = 0;
        ModernDialog.StyleSelectionInput(_dashboardStatusFilter);
        _dashboardStatusFilter.SelectedIndexChanged += (_, _) => RefreshDashboard();
        ...
        UiTheme.ApplyRoundedCorners(_dashboardShowAllProfilesToggle, 10);
        UpdateDashboardProfileFilterToggleStyle();
        var refresh = new Button { Text = "↻", Dock = DockStyle.Fill, Margin = Padding.Empty };
        ...
        tools.Controls.Add(_dashboardSearchBox, 0, 0);
        tools.Controls.Add(_dashboardStatusFilter, 1, 0);
        tools.Controls.Add(_dashboardShowAllProfilesToggle, 2, 0);
        tools.Controls.Add(refresh, 3, 0);
        ...
        UiTheme.ApplyRoundedCorners(emptyIcon, 38);
        ...
        var open = new Button { Text = "+ Mở hồ sơ", AutoSize = true };
        var create = new Button { Text = "Tạo hồ sơ mới", AutoSize = true };
    ...
    Panel BuildUpdatePanel()
    {
        ...
        panel.Controls.Add(updateIcon);
        panel.Controls.Add(updateTitle);
        ...
        void LayoutUpdateControls()
        {
            ...
            _dashboardUpdateStatus.Width = Math.Max(180, Math.Min(380, panel.ClientSize.Width - 88));

            var right = Math.Max(0, panel.ClientSize.Width - rightPadding);
            configure.Left = Math.Max(10, right - configure.Width);
            ...
            var selectorRight = Math.Max(300, _dashboardUpdateButton.Left - gap);
            _dashboardVersionSelector.Left = Math.Max(300, selectorRight - 230);
            _dashboardVersionSelector.Width = Math.Max(170, selectorRight - _dashboardVersionSelector.Left);

            updateTitle.Visible = panel.ClientSize.Width >= 760;
            updateIcon.Visible = panel.ClientSize.Width >= 760;
            _dashboardUpdateStatus.Visible = panel.ClientSize.Width >= 760;
        }
    ...
    Panel BuildDashboardActions()
    {
        ...
        Button ActionButton(string glyph, string titleText, string description, Color accent, Func<ProfileContext, Task> action)
        {
            var b = new Button
            {
                Text = $"{glyph}  {titleText}   ›{Environment.NewLine}     {description}",
                AutoSize = false,
                Width = 170,
                Height = 56,
                Margin = new Padding(0, 0, 10, 0),
                TextAlign = ContentAlignment.MiddleLeft,
                Padding = new Padding(10, 3, 6, 3)
            };
            UiTheme.StyleButton(b, UiButtonKind.Neutral);
            b.BackColor = Color.White;
            b.ForeColor = accent;
            b.FlatAppearance.BorderColor = UiTheme.Border;
            b.FlatAppearance.MouseOverBackColor = UiTheme.PrimarySoft;
            b.Font = new Font("Segoe UI Semibold", 8.75F, FontStyle.Bold);
            b.Click += async (_, _) =>
            {
        ...
        flow.Controls.Add(ActionButton("▰", "Mở hồ sơ", "Mở vùng làm việc", UiTheme.Primary, async ctx =>
        {
            await OpenProfileAsync(ctx);
            if (ctx.Tab is not null) SelectTabPageSafely(ctx.Tab);
        }));
        flow.Controls.Add(ActionButton("◉", "Hiện Chrome", "Mở cửa sổ trình duyệt", Color.FromArgb(37, 99, 235), ViewChromeForProfileAsync));
        flow.Controls.Add(ActionButton("▶", "Bắt đầu chạy", "Khởi động hồ sơ đã chọn", UiTheme.Success, async ctx =>
        {
            await OpenProfileAsync(ctx);
            await StartWithNameGuardAsync(ctx, "start", TimeSpan.FromSeconds(30));
        }));
        flow.Controls.Add(ActionButton("Ⅱ", "Tạm dừng / Tiếp tục", "Đổi trạng thái vận hành", Color.FromArgb(217, 119, 6), async ctx =>
        {
        ...
        flow.Controls.Add(ActionButton("■", "Dừng chạy", "Dừng hồ sơ đã chọn", UiTheme.Danger, async ctx =>
        {
            if (ctx.Worker is not null && !ctx.Worker.HasExited)
                await SendCommandAsync(ctx, "stop", TimeSpan.FromSeconds(8));
        }));
        flow.Controls.Add(ActionButton("↻", "Khởi động lại Chrome", "Tải lại trình duyệt", UiTheme.Purple, async ctx =>
        {
            if (ctx.Worker is null || ctx.Worker.HasExited)
                await OpenProfileAsync(ctx);
